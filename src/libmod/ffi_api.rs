...
fn ffi𝑒( /// Return an FFI-friendly error string to a non-Rust caller
  ...
  s: impl AsRef<str>,) -> 𝑝𝑠 { U16CString::from_str(s).map(U16CString::into_raw)
    .unwrap_or_else(|𝑒|{// eprintln!("Error, `s` returned a string with inner null bytes: {}", 𝑒);
...
fn dealloc_lib_string(𝑝:𝑝𝑠) {unsafe {
  if 𝑝.is_null() { return; } else { drop::<U16CString>(U16CString::from_raw(𝑝)); }
}}
